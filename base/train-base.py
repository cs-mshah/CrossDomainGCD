...
from utils.utils import AverageMeter, accuracy, set_seed, save_checkpoint, sim_matrix, interleave, de_interleave, describe_splits
...
def main():
    ...
    args.train_split = 0.8 # if dann true, then this is cross_domain train split (as train domain uses full), else same domain train split
    # cross domain args
    args.dann = False
    # args.alpha = 0.20
    # args.alpha_exp = False
    args.train_domain = 'RealWorld'
    ...
    if args.resume:
        ...
        simnet.load_state_dict(checkpoint['state_dict_simnet'], strict=False)
        optimizer_simnet.load_state_dict(checkpoint['optimizer_simnet'])

    # wandb init
    wandb.init(entity='cv-exp', 
               project='IITB-MBZUAI', 
               name=f'officehome same domain 100% known 100% labeled{args.run_started}',
               config=args,
               group='officehome',
