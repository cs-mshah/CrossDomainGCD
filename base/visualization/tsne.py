...
from utils.utils import set_seed, describe_image_dataset
from datasets.datasets import get_tsne_dataset
...
def main():
    ...
    exp_name = f'dataset_{args.dataset}_arch_{args.arch}_lbl_percent_{args.lbl_percent}_novel_percent_{args.novel_percent}_closed_wordl_ssl_{args.cw_ssl}_{args.description}_split_id_split_{args.split_id}_{run_started}'
    args.out = os.path.join(base_path, args.out, exp_name)
    ...
    train_dataset, test_dataset = get_tsne_dataset(args)
    ...
    # palette = sns.color_palette("Spectral", 16)

    plot = sns.scatterplot(x=vis_x, y=vis_y, hue=labels[:,0], legend='full')
    filename = os.path.join(args.fig_root, exp_name + '.png')
